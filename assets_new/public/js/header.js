...
// Maximum exceeded
    ...
    $(document).on('keyup', '.indesc_count', function() {
        var limit = 150;
        var text_remaining;
        var str = $(this).attr('id');
        var idd = str.split("**");
        var id = idd[1];
        var text_length = document.getElementById('indesr**' + id).value.length;
        text_remaining = limit - text_length;
        if (text_remaining == limit) {
            document.getElementById('indese' + id).innerHTML = limit;
        } else {
            if (text_remaining >= 0) {
                document.getElementById('indese' + id).innerHTML = text_remaining;
            } else if (text_remaining < 0) {
...
//gamechat 
    var lim = 150;
    var text_remaining;
    $(document).on('keyup', '#main_title_gamechat', function() {


        var text_length = document.getElementById('main_title_gamechat').value.length;
        text_remaining = lim - text_length;
        if (text_remaining == 150) {
            document.getElementById('title_count_gamechat').innerHTML = '<span >150</span>';
        } else {
            if (text_remaining >= 0) {
                document.getElementById('title_count_gamechat').innerHTML = '<span>' + text_remaining + '</span>';
            } else if (text_remaining < 0) {

                document.getElementById('title_count_gamechat').innerHTML = '<span><font color=red>' + text_remaining + '</font></span>';
            }
        }
...
$("#fb_login_model").click(function() {
    $('#login_close').trigger('click');
});
$("#fb_login_model2").click(function() {
    $('#sign_close').trigger('click');
});
...
//    var category = $(".pic_category:checked").val();
    ...
    if ($("#in_poll_spoiler").is(':checked')) {
        var spoiler = 1;
    } else {
        var spoiler = 0;
    }
    var creditChk = $("#credit_poll_author").is(':checked');
    if ($("#credit_poll_author").is(':checked')) {
        var credit = $("#poll_credit").val();
        var author = $("#poll_author").val();
    } else {
        var credit = "";
        var author = "";
    }
...
//                            $("#first_poll_alert").html('<strong>' + data.msg + '</strong>');
                ...
                $('#inline_poll_error_author').html(data.author);
                $('#inline_poll_error_credit').html(data.credit);
//                $("#first_poll_alert").show();
//                $("#first_poll_alert").html('<strong>' + data.msg + '</strong>');
            }
        }
...
//        },
//        success: function(msg) {
//            $('#category_gamechat_list').html(msg);
//            $("#modal_upload").hide();
//            $(".gamechat-upload").show();
//        }
//    });
...
$("#gameform").submit(function(event) {
    event.preventDefault();
//    var category = $("#category_gamechat_list .pic_category:checked").val();
    var splashart_model = $("#splashart_model").val();
    var main_title = $("#main_title_gamechat").val();
    $.ajax({type: "POST",
...
//            category: category,
        ...
        success: function(data) {
            if (data.result == "success") {
                ...
                $('.popupContainer').animate({scrollTop: 0}, 'fast');
                $("#gamechat_alert").show();
                $("#gamechat_alert").html('<strong>' + data.msg + '</strong>');
            }
        }});
});

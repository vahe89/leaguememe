...
function discussion_fav(favourites) {
    $.ajax({
        type: "POST",
        url: base_url + "public/animelist/anime_favourites",
        ...
        success: function(data) {
            if (data.status == "false") {
                ...
                $('#disFav_' + favourites).attr('src', base_url + 'assets/public/img/icon/post/open.png'); 
                $('#disFav_' + favourites).closest(".wrapper-avatar").remove();

            } else if (data.status === "insert") {
                $('#disFav_' + favourites).attr('src', base_url + 'assets/public/img/icon/post/open-hover.png');

...
function loadDisc($tab) {
    $.ajax({
        url: "public/animelist/discussion_list",
        data: {
            anime_name: 0,
            headerType: 1,
            main: $tab
        ...
        dataType: 'HTML',
        beforeSend: function(xhr) {
            $("#all-discussion").html('<div style="text-align: center; padding-top: 80px"><img src="' + base_url + 'assets/public/img/ajax-loader.gif" /></div>');
        },
        success: function(data, textStatus, jqXHR) {
            $("#all-discussion").html(data);
        },
        error: function(jqXHR, textStatus, errorThrown) {
            $("#all-discussion").html('<div style="text-align: center; padding-top: 80px">Error while loading discussion.</div>');
        }
    })
}

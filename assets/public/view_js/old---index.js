...
function toggleSlides() {
    ...
    $('.toggler').click(function (e) {
        ...
        // $('.innercomboBox').keyup(function(){
        var parentid = $('.innercomboBox').parent().attr('id');

        $('.replyBox').each(function () {
            var id = $(this).attr('id');
            var replyboxid = id.substring(id.indexOf('-') + 1, id.length);
            $('#addrplCommentBox'+replyboxid).keyup(function () {
                var text_length = $('#addrplCommentBox'+replyboxid).val().length;
                inertext_remaining = limit - text_length;
                if (inertext_remaining == 1000) {
                    $("#inerwordcountdiv" + replyboxid).html('<p id=inerwordcount'+ replyboxid +'>1000</p>');
                } else {
                    if (inertext_remaining >= 0)
                        $("#inerwordcountdiv" + replyboxid).html('<p id=inerwordcount' + replyboxid + '>' + inertext_remaining + '</p>');
                    else if (inertext_remaining < 0) {
                        $("#inerwordcountdiv" + replyboxid).html('<p id=inerwordcount' + replyboxid + '><font color=red><b>' + inertext_remaining + '</b></font></p>');
                    }
                }
            });
            //$("#addrplCommentBox")
